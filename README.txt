Gameplay Options
    - Coach
        - You are a coach of a choice school (or create your own)
        - Your tasked to draft players, increase ratings, and build your schools prestige
    - Player
        - You are a Freshman choosing a school with dreams of becoming a college basketball player
            - You have 4 years to raise your draft projections



Ideas




March Madness
- Create each matchup as a node
    - If a team wins, the next game points to the next node
    - You could create a variable "tournamentWins" to add to a teams prestige

Awards at the end of season (pre tournament)
    - Best Defense
        - Team with lowest points against
    - Best Offense
        - Team with most points
    - Best +/-
        - Team with best points vs points agianst ratio
        (I feel this would be the number 1 seed)

Team Legacy
    - Prestige level
    - Tradition Ranking
    - Education Ranking

Quality
    - Option to enter teams/players based on either:
        Input
        File(s)

Ratings 
- Influence of Team Rating
    - Record (losing)
        - Better players will want a higher win percentage
        - If it doesn't meet their wants they will try to transfer*
- Influence of Player Rating 
    - Better coaching can increase player ratings
    - A bad game can decrease a player ratings
        - Better players will be decreased for games under a certain amount of points
        - Worse players will be increased for games over a certain amount of points

Transfer Portal*

*Home Court Advantage

Playoffs (Rounds determined by seed)
- 16 Team Playoff
Round 1
    Game 1
        (Seed 1 vs Seed 16)
    Game 2
        (Seed 2 vs Seed 15)
    Game 3
        (Seed 3 vs Seed 14)
    Game 4
        (Seed 4 vs Seed 13)
    Game 5
        (Seed 8 vs Seed 9)
    Game 6
        (Seed 7 vs Seed 10)
    Game 7
        (Seed 5 vs Seed 12)
    Game 8
        (Seed 6 vs Seed 11)
Round 2
    Game 9
        (Winner of Game 1 vs Winner of Game 5)
    Game 10
        (Winner of Game 2 vs Winner of Game 6)
    Game 11
        (Winner of Game 3 vs Winner of Game 7)
    Game 12
        (Winner of Game 4 vs Winner of Game 8)
Round 3
    Game 13
        (Winner of Game 9 vs Winner of Game 11)
    Game 14
        (Winner of Game 10 vs Winner of Game 12)
National Championship (Round 4)
    Game 15
        (Winner of Game 13 vs Winner of Game 14)
